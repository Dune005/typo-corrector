<template>
    ...
    <button
      class="correct-button"
      @click="debouncedCorrectText"
      :disabled="isLoading || !inputText.trim()"
    >
      {{ isLoading ? 'Correcting...' : 'Refresh Correction' }}
    </button>
  </div>
  ...
  <script setup>
    ...
    // Backend URL definieren
    const backendUrl = 'http://localhost:5001/correct';
  
  // Debounce Funktion um zu häufige API-Aufrufe zu vermeiden
  ...
  // Debounced Version der correctText Funktion
  const debouncedCorrectText = debounce(async () => {
  ...
  // Watch für automatische Korrektur
  watch(inputText, () => {
    debouncedCorrectText();
  });
  </script>
